...
function assertEip712Transaction(transaction) {
  const { chainId, to, from: from10, paymaster, paymasterInput } = transaction;
  ...
  if (from10 && !isAddress(from10))
    throw new InvalidAddressError({ address: from10 });
...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce, to, from: from10, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce, to, from: from10, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from10),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from10, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from10,
    to ?? "0x",
...
function assertTransactionDeposit(transaction) {
  const { from: from10, to } = transaction;
  if (from10 && !isAddress(from10))
    throw new InvalidAddressError({ address: from10 });
  if (to && !isAddress(to))
    throw new InvalidAddressError({ address: to });
...
function assert3(envelope) {
  ...
  if (type2 === "p256") {
    const p256 = envelope;
    const missing = [];
    if (typeof ((_a = p256.signature) == null ? void 0 : _a.r) !== "bigint")
      missing.push("signature.r");
    if (typeof ((_b = p256.signature) == null ? void 0 : _b.s) !== "bigint")
...
function hash(authorization, options = {}) {
  ...
  return keccak256(concat("0x05", fromHex(toTuple2(presign ? {
...
function from6(tokenIdOrAddress) {
...
function from7(value) {
  return keccak256(concat(padLeft(toAddress(value.userToken), 32), padLeft(toAddress(value.validatorToken), 32)));
}
...
function serialize2(role) {
  ...
  return keccak256(fromString(toPreHashed[role] ?? role));
}
...
function fromEther(ether, unit = "wei") {
  return from8(ether, exponents.ether - exponents[unit]);
}
function fromGwei(gwei, unit = "wei") {
  return from8(gwei, exponents.gwei - exponents[unit]);
}
...
function from9(envelope, options = {}) {
...
function formatTransaction2(transaction) {
  ...
  const { feePayerSignature, gasPrice: _, nonce, ...tx } = Transaction_exports2.fromRpc(transaction);
  return {
    ...tx,
...
var chainConfig5 = {
  ...
  prepareTransactionRequest: [
    async (r, { phase }) => {
      ...
      if (!request.feeToken && ((_a = request.chain) == null ? void 0 : _a.feeToken))
  ...
  async verifyHash(client, parameters) {
    ...
    if (typeof signature === "string" && signature.endsWith(magicBytes.slice(2))) {
      const envelope = deserialize(signature);
...
// node_modules/viem/_esm/chains/definitions/tempoTestnet.js
...
//# sourceMappingURL=chunk-LIEGBX6Y.js.map
